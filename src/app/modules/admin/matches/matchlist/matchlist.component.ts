...
export class MatchlistComponent implements OnInit {
    ...
    onfirstFormSubmit() {
        if (this.firstForm.valid) {
            ...
            var sendData = {
                matchName : this.firstForm.value.matchName
            }
            const encryptedRequest = this.encrypt.encryptData(
                JSON.stringify(sendData)
            );
            this.matchservice.addMatch(encryptedRequest).subscribe(
                (data: any) => {
    ...
    getList() {
        ...
        this.matchservice.getMatchList(encryptedRequest).subscribe(
            (data: any) => {
                /*setTimeout(() => {
                    this.spinner.hide();
                }, 200);*/
    ...
    setData(data) {
        this.router.navigate(['admin/matches/detail'], {
            queryParams: { tournmentId: data.tour_id },
        });
    }
    changeStatus(row) {
        console.log(row);
