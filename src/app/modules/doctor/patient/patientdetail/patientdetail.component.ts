...
import { ActivatedRoute, Router, NavigationStart, NavigationEnd, NavigationError, RouterEvent } from '@angular/router';
...
export type ChartOptionsLineOne = {
    series: ApexAxisChartSeries;
    chart: ApexChart;
    xaxis: ApexXAxis;
    stroke: ApexStroke;
...
export class PatientdetailComponent implements OnInit {
    ...
    @ViewChild('deleteReason') deleteReasonDialog: TemplateRef<any>;
    ...
    public chartOptionsLineOne: Partial<ChartOptionsLineOne>;
    chartOptionsLineEnalbeOne = true;
  currentRoute: string;
    constructor(
        ...
        /*this.renderer.listen('window', 'click',(e:Event)=>{
            console.log('somu test checking');
        });*/
      ...
      if (location.path() == "/doctor/patient-list") {
    ...
    /*this.router.events
      .pipe(
        ...
        if(location.path().split("?")[0]!='/doctor/patient-detail'){
    ...
    reasonList = [{name: 'Treatment is no longer required',value:0},{name: 'The Patient has moved to a new location',value:1},{name: 'The patient has changed the provider',value:2},{name: 'Others',value:3}];
    reason: any;
    tyears: any = [];
    monthSelected: any;
    ...
    ngOnInit(): void {
        /*const element = document.querySelector('#graphSection');
element.scrollIntoView();*/
    ...
    getPatientData() {
      ...
      if(value.date!=null){
          ...
          firstName : 'aa',
                    ...
                    this.firstForm.controls['hubnumber'].setValue(
                        this.patientDetailData.exclude_hub_number
                    );
                    if(this.patientDetailData.kittingstatus>1){
                      this.firstForm.controls['hubnumber'].disable();
                    }else{
                      this.firstForm.controls['hubnumber'].enable();
                    }
                    this.imgURL = this.patientDetailData.patient_image;
                    console.log('somu date checl');
                    console.log(new Date(this.patientDetailData.created_at) );
                    this.kitorderdate =
                        this.patientDetailData.created_at == '' ||
                        this.patientDetailData.created_at == null
                            ? '-'
                            : this.patientDetailData.created_at;
    ...
    onDeletePatient(){
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'confirmed') {
                let dialogRef = this.elements.open(this.deleteReasonDialog, {
                width: '420px',
            });
            this.deletePatient();
                // console.log('confirm');
    ...
    deletePatient(){
        ...
        var sendData = {
            ...
            notes : this.deleteForm.value.notes
        };
        const encryptedRequest = this.encrypt.encryptData(
    ...
    getTimeRecorded() {
        ...
        this.service.getPatientRecordTime(encryptedRequest).subscribe(
            (data: any) => {
                ...
                console.log('total time'+this.totalTimeSpend);
                ...
                this.totalTimeRemaining =
                    ...
                    console.log("current"+this.totalTimeSpend);
                ...
                if (this.totalTimeSpend >= 3600) {
                    ...
                    this.colorFill = '#1255A8';
                    this.heartValue = -((this.totalTimeSpend / 60)*5.4)+30;
                }
                /*if (this.totalTimeSpend < 1200) {
                    this.blueColor = (this.totalTimeSpend / 60) * this.pi;
                    this.yelloColor = 0;
                } else if (this.totalTimeSpend < 2400) {
                    this.blueColor = (1200 / 60) * this.pi;
                    var yellowValue =  this.totalTimeSpend - 1200;
                    this.yelloColor = (yellowValue / 60) * this.pi;
                } else if (this.totalTimeSpend < 3600){
                    this.blueColor = (1200 / 60) * this.pi;
                    var yellowValue =  this.totalTimeSpend - 1200;
                    this.yelloColor = (yellowValue / 60) * this.pi;
                } else {
    ...
    removeSpace(event, modelName) {
        ...
        if (modelName == 'modelnotes') {
            this.modelnotes = event.trim();
        }
    ...
    onfirstFormSubmit() {
      ...
      console.log(sendData);*/
        ...
        formData.append('order_shipping_date',order_shipping_date);*/
            ...
            formData.append('hubnumber', this.firstForm.value.hubnumber);
            //const encryptedRequest = this.encrypt.encryptData(JSON.stringify(formData));
    ...
    getGraphReading(value) {
        ...
        this.service.getPatientReadingsGraph(encryptedRequest).subscribe(
            (data: any) => {
                ...
                this.lineChartLabels = data.data.month;*/
                var dateTimeTemp = [];
                 data.dataDay.dateTime.forEach((value) => {
                    value = moment
                        .unix(value / 1000)
                        .format('MM/DD/YYYY, hh:mm:ss a');
                        console.log(value);
                    dateTimeTemp.push(value);    
                });
                this.chartOptionsLineOne = {
                            series: [
                                {
                                    name: 'Systolic',
                                    data: data.dataDay.systolicDay,
                                    /*data: [
                                        47, 57, 74, 89, 75, 38, 62, 47, 82, 56, 45,
                                        47,
                                    ],*/
                                },
                                {
                                    ...
                                    data: data.dataDay.diastolicDay,
                                },
                            ],
                            chart: {
                                toolbar: {
                            ...
                            xaxis: {
                                ...
                                categories: dateTimeTemp,//data.dataDay.dateTime,
                            },
                            tooltip: {
                                y: [
                                    {
    ...
    onDeleteReason(value){
        ...
        if(this.reason==3){
            this.deleteForm.get('notes').setValidators([Validators.required]);
            this.deleteForm.get('notes').updateValueAndValidity();
        }else{
            this.deleteForm.get('notes').clearValidators();
            this.deleteForm.get('notes').updateValueAndValidity();
        }
    }
    animateAnalogClock() {
    ...
    getListToDo() {
        ...
        this.service.getToDoList(encryptedRequest).subscribe(
            (data: any) => {
                ...
                this.totaltodo = this.encryptedResponse.data.systemTotalvalue;
                this.opencount = this.encryptedResponse.data.open;
                this.closecount = this.encryptedResponse.data.close;
                this.pagetodo.totalElements = this.totaltodo;
